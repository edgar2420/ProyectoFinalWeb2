...
exports.createEvolucion = async (req, res) => {
    try {
        ...
        console.log("Archivos recibidos:", req.files); // Verifica si el archivo está siendo recibido
        ...
        if (req.files && req.files.imagen) {
            ...
            // Cambia la ruta de subida para que sea directamente en `uploads/`
            const uploadPath = path.join(__dirname, '../uploads/', imagen.name);

            console.log("Ruta de subida de imagen:", uploadPath);  // Verificar si la ruta de subida es correcta
...
exports.getEvolucionByPokemonId = async (req, res) => {
    try {
        ...
        // Buscar las evoluciones relacionadas con el Pokémon
        const evoluciones = await db.evolucion.findAll({
