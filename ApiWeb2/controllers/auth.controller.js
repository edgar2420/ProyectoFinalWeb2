...
exports.generateUserToken = async (req, res) => {
    ...
    // Generar un token y actualizar el campo del token en la base de datos
    const token = generarTokenUsuario();
    console.log("Generated token: ", token); // Debug

    try {
        // Guardar el token en la tabla usuarioauths
...
exports.registerAdmin = async (req, res) => {
    ...
    // Crear el usuario en la base de datos con rol de administrador
    const usuario = await db.usuarios.create({
        nombre,
        email,
        password: stringToSha1(password),
        esAdmin: true // Asignar rol de administrador
    });
    usuario.password = undefined;
...
exports.me = async (req, res) => {
    // Obtener la información del usuario actual desde res.locals
    const usuario = await db.usuarios.findOne({
        where: {
    ...
    try {
        const usuarios = await db.usuarios.findAll({
            include: [
                ...
                { model: db.movimientos, as: 'movimientos' } // Si los movimientos son directamente relacionados con usuarios
            ]
        });
        res.send(usuarios);
    } catch (error) {
